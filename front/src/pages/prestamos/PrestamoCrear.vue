            ...
            <q-card flat bordered>
              ...
              <q-markup-table flat bordered dense>
                ...
                <tr v-for="cli in clientes" :key="cli.id" :class="{ 'bg-blue-2': prestamo.cliente_id === cli.id }">
                  ...
                  <td>
                    <q-btn label="Seleccionar" color="primary" dense no-caps icon="check" @click="seleccionarCliente(cli)" />
                  </td>
                </tr>
                </tbody>
              </q-markup-table>
              ...
              <q-card-section class="q-pa-xs">
                <q-form @submit.prevent="guardarPrestamo">
                  <div class="row q-col-gutter-sm">
                    ...
                    <div class="col-6 col-md-4">
                      <q-input label="Prestado (Bs)" v-model.number="prestamo.valor_prestado" type="number" outlined dense @update:model-value="calcularSaldo"/>
                    </div>
                    <div class="col-6 col-md-4">
                      <q-input label="Interés (Bs)" v-model.number="prestamo.interes" type="number" outlined dense @update:model-value="calcularSaldo"/>
                    </div>
...
export default {
  ...
  data () {
    return {
      ...
      prestamo: {
        ...
        peso: 0, valor_total: 0, valor_prestado: 0, interes: 0, saldo: 0,
        celular: '', detalle: '', cliente_id: null, cliente: null
      }
    }
  },
  async mounted () {
  ...
  methods: {
    ...
    calcularSaldo () {
      this.prestamo.saldo = (this.prestamo.valor_prestado || 0) + (this.prestamo.interes || 0)
    },
    guardarPrestamo () {
