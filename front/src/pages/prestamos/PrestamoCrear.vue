                  ...
                  <div class="row q-col-gutter-sm">
                    ...
                    <div class="col-6 col-md-3">
                      <q-select
                        ...
                        :options="[1,2,3]"
                        @update:model-value="calcularSaldo"
                        :readonly="$store.user.role !== 'Administrador'"
                      />
                    </div>
...
export default {
  ...
  data () {
    return {
      ...
      precioOro: { value: 0 }, // viene de cogs/3 (precio compra)
  ...
  watch: {
    // Si por alguna razón cambia el precio, recalcule
    precioOro: {
      deep: true,
  ...
  methods: {
    ...
    calcularTotales () {
      ...
      // La merma no puede superar al peso bruto
      if (merma > bruto) {
        merma = bruto
      ...
      // Valor referencial = peso neto * precio de compra del oro
      const precio = Number(this.precioOro?.value || 0)
      this.prestamo.valor_total = +(neto * precio).toFixed(2)

      // Recalcular totales derivados del valor prestado
      this.calcularSaldo()
    },
    ...
    calcularSaldo () {
      ...
      // <!--                      almacenaje 2% mayo o igual a mill y menos 3% No se modifica (se modifica para el administrador)-->
    ...
    guardarPrestamo () {
      ...
      this.$axios.post('prestamos', {
        ...this.prestamo,
        client_id: this.prestamo.cliente_id,
