import{h as d}from"./moment-C5S46NFB.js";import{P as h}from"./index-C5E5KQtJ.js";const x={nombre:"JOYERIA ROSARIO",sucursal:"ORURO",direccion:"DIR. ADOLFO MIER, POTOSI Y PAGADOR (LADO PALACE HOTEL)",cel:"73800584",telefono:"52-55713"},u=`
...
`;function a(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function o(e){return Number(e||0).toFixed(2)}function v(e,t="DD/MM/YYYY"){return e?d(e).format(t):"-"}function f(e){const t=e?.defaults?.baseURL||window.location.origin,i=new URL(t,window.location.origin);return i.pathname=i.pathname.replace(/\/api\/?$/,"/"),i.toString().replace(/\/$/,"")}function r(e,t){return t?`${e}/images/${t}`:""}function p(e,t,i){const s=f(i),n=d(e?.fecha_creacion||new Date),l=d();return{company:x,logo:r(s,"logo.png"),fallbackRings:r(s,"rings.png"),photo:r(s,e?.foto_modelo),numero:e?.numero||"",cliente:e?.cliente?.name||"N/A",celular:e?.cliente?.cellphone||"N/A",detalle:[e?.detalle||"-",`Peso: ${e?.peso||0} gr.`,`Oro: ${e?.kilates18||"-"}`].join(" / "),costoTotal:o(e?.costo_total),adelanto:o(Number(e?.adelanto||0)+Number(e?.totalPagos||0)),saldo:o(e?.saldo),entrega:e?.fecha_entrega?v(e.fecha_entrega):"-",observaciones:e?.observacion||e?.observaciones||"",nota:e?.nota||"Ningun trabajo sera entregado sin la presente orden. Importante en caso de no recojo se espera un maximo de 90 dias antes de proceder a la fundicion de la joya",garantia:{codigo:e?.numero||"-",fecha:l.format("DD/MM/YYYY"),dia:l.format("DD"),mes:l.format("MM"),ano:l.format("YYYY"),cliente:e?.cliente?.name||"N/A",tipo:"Joya",periodo:"1 ano",detalle:e?.detalle||"-",mantenimientoMeses:12,precioOro:o(t)},fechaTrabajo:{hoy:l.format("DD/MM/YYYY"),dia:n.format("DD"),mes:n.format("MM"),ano:n.format("YYYY")}}}function c(e,t="top"){console.log(e);const i=e.photo?`<div class="reference-box"><img class="reference-image" src="${a(e.photo)}" alt="Foto de referencia"></div>`:`
    ...
    <div class="sheet order-copy-${a(t)}">
      ...
      <table class="grid" style="padding-top: 20px">
        <tr>
      ...
      <table class="grid">
        <tr>
          ...
          <td class="cell observaciones-box" style="width:60%">
            ...
            <div class="center large bold">${a(e.celular)}</div>
          </td>
        </tr>
      </table>
